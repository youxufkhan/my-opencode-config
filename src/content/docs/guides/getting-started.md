...
description: Learn how to set up and use my-opencode-config.
---

# Getting Started with my-opencode-config

Welcome to `my-opencode-config`! This CLI tool is designed to help you quickly and easily set up your OpenCode environment with the best free and powerful models, essential plugins, and optimal configurations.
...
The fastest way to use `my-opencode-config` is via `npx`. No installation is required!
...
When you run `my-opencode-config`, it will guide you through an interactive 8-step process:

1.  **Check OpenCode**: Verifies your OpenCode installation.
2.  **Essential Plugins**: Automatically adds `oh-my-opencode-config` and `opencode-gemini-auth`.
...
8.  **Configuration Generation**: Generates your `opencode.json` and `oh-my-opencode-config.json` files.
...
- **Learn more**: Check out our [Features Guide](/reference/features/) for a deeper dive into model scoring and plugin management.
